...
public class ordersModule {
	...
	public String jbxx(String pai, String gongsiNo, String caozuoyuan_xm) {
		...
		if (result.size() == 0) {
			...
			if(che!=null){
				...
				che_new.setChe_wxys(che.getChe_wxys());
				che_new.setChe_nf(che.getChe_nf());
				dao.insert(che_new);
	...
	@At
	@Ok("raw:json")
	public String addjb(@Param("..") Work_pz_gzEntity yuyue) {
		yuyue.setMainstate(-1);
		System.out.println(yuyue.getXche_sffl()+"============================="+yuyue.getXche_sfbz());
		dao.update(yuyue,"^card_no|che_wxys|che_nf|xche_bz|xche_cy|xche_lc|che_cx|che_vin|xche_lc|kehu_mc|kehu_dh|xche_sfbz|xche_sffl|xche_gj|mainstate$");
		if (yuyue.getChe_no() != null) {
			Work_cheliang_smEntity che = pu.saveCheInfose(yuyue.getXche_bz(),yuyue.getChe_no(), yuyue.getGcsj(),
					yuyue.getChe_cx(), yuyue.getChe_vin(),yuyue.getChe_nf(),yuyue.getChe_wxys());
			System.out.println(yuyue.getKehu_mc()+"============================="+yuyue.getKehu_dh());
			pu.saveKeHu(che.getKehu_no(), yuyue.getKehu_mc(),
					yuyue.getKehu_dh());
	...
	public String wx_jinchang(String work_no) {
		...
		Sql sql2 = Sqls
				.create("update work_pz_gz set flag_fast=0, xche_wxsj = isnull(DATEDIFF(minute, xche_jdrq, xche_yjwgrq),0) where work_no= '"
						+ work_no + "'");
		dao.execute(sql2);
...
//						+ pz.getChe_no() + "' and work_no='" + work_no + "'");
//		dao.execute(sql6);
	...
	@At
	@Ok("raw:json")
	public String dayin(String work_no) throws UnsupportedEncodingException {
		//没有判断work_no是否为空
		String xche_cz="";
		Sql sql = Sqls
				.queryRecord("select xche_cz from work_pz_gz where work_no = '"+work_no+"'");
		dao.execute(sql);
		...
		if (res3.size() > 0) {
			...
			Sql sql2 = Sqls
					.queryRecord("select printpc from PrintServerset where billkind = '接待登记单'");
			dao.execute(sql2);
			...
			if (res2.size() > 0) {
				...
				Sql sql1 = Sqls
						.queryRecord("insert into PrintServerLog(printdate,billkind,billno,printczy,printpc,printsource,flag_print ) values (getdate(),'接待登记单','"
								+ work_no
								+ "','"
								+ xche_cz
								+ "','"
